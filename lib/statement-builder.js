 ...
 *
 *  @summary Explicitly inserts single quotes around strings unless they contain a single quote, in which
 *  case inserts double quotes around strings.
 *
 *  @since 1.0.0
 *
...
var _createStateProxy = function () {
  ...
  return Object.create({
    ...
    insertConjunction: function(conjunction) {
      console.log("conjunction is: " + conjunction)
      var _whereStm = this._whereStm
      if (!_.isNull(conjunction) && !_.isUndefined(conjunction)) {
        _initWhereStmt(this)
        if (!_.endsWith(_.trimEnd(_whereStm), 'WHERE') && !_.endsWith(_whereStm, '(')) {
          this._whereStm += conjunction
        }
      }
    },
    parseComplexSelectOperator: function(key, value) {
      ...
      for (var op in value) {
        ...
        _count += 1
        _params = value[op]
//        _valPlaceholder = key + '_' + _count
        var _valPlaceholder = _fieldPlaceholder(this, key, _count)
        if (!_.isEmpty(_operatorStm)) {
          _operatorStm += ' AND '
        }
...
console.log("inserting conjunction 1")
          _stateProxy.insertConjunction(_stateProxy.conjunction)
        }
...
console.log("inserting conjunction 2")
          _stateProxy.insertConjunction(_stateProxy.conjunction)
          try {
            if (value['nin$']) {
...
//            _stateProxy.whereStatement += _parseComplexSelectOperator(key, value)
          ...
          if (!_stateProxy.handled) {
            if (!_.isNull(value) && !_.isUndefined(value)) {
//              _stateProxy.whereStatement = _handleConjunctionInsertion(_stateProxy.conjunction, _stateProxy.whereStatement)
console.log("inserting conjunction 3")
              _stateProxy.insertConjunction(_stateProxy.conjunction)
